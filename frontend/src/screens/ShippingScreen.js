...
const ShippingScreen = ({ history }) => {
  ...
  const checkDeliveryHandler = useCallback(
    async (val) => {
      setPostalCode(val);
      if (val.length === 6) {
        setAlert("Processing...");
  ...
  const updatePhoneNumber = async (val) => {
    setPhone(val);
    if (val.length === 10) dispatch(updatePhone(val));
  };
  ...
  return (
    <FormContainer>
      ...
      <Form onSubmit={submitHandler}>
        ...
        <Button
          type="submit"
          disabled={!city || !(phone.length === 10) || phoneError}
          variant="primary"
          className="bg-danger"
        >
      ...
      {alert && alert !== "Processing..." && pincode && (
        <Alert
          variant={deliverable && !phoneError ? "success" : "danger"}
